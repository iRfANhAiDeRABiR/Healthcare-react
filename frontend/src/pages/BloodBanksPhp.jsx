...
export default function BloodBanksPhp() {
  ...
  const activeDonors = useMemo(() => {
    return donors.filter((donor) => donor.available === true || Number(donor.available) === 1);
  }, [donors]);
  ...
  async function refreshCurrentView(nextView = view) {
    await safeLoad(async () => {
      ...
      if (nextView === "donors" || nextView === "activeDonors") await loadDonors();
  ...
  function go(nextView) {
    ...
    if (nextView === "activeDonors") safeLoad(loadDonors, "Could not load active donors.");
        ...
        {view === "dashboard" && (
          <>
            ...
            <div className="bb-modern-actions">
              ...
              <ActionCard color="blue" icon={<Users />} title="View Donors" text="View only currently active / available donors" onClick={() => go("activeDonors")} />
        ...
        {view === "activeDonors" && (
          <>
            <div className="bb-card">
              <BackButton go={go} />
              <h2>Active Donors</h2>
              <p className="bb-small">Only donors currently marked as available for donation are shown here.</p>
            </div>

            <div className="bb-filter-summary">Showing {activeDonors.length} active donor{activeDonors.length === 1 ? "" : "s"}.</div>
            {!activeDonors.length && <div className="bb-card">No active donors found right now.</div>}
            {activeDonors.map((donor) => (
              <div className="bb-donor bb-active-donor" key={donor.id}>
                <div>
                  <h3>{donor.name}</h3>
                  <p className="bb-small">{donor.district || "Unknown District"}, {donor.division || "Unknown Division"}</p>
                  <p className="bb-small">Contact: {donor.contact || donor.phone}</p>
                </div>
                <div>
                  <span className="bb-badge">{donor.blood_group}</span>
                  <span className="bb-status bb-accepted">Active</span>
                  <p className="bb-small">Donations: {donor.total_donations || 0}</p>
                </div>
                <div>
                  <button className="bb-btn" type="button" onClick={() => startDirectRequest(donor)}>Send Request</button>
                </div>
              </div>
            ))}
          </>
